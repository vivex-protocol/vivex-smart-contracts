...
contract OrderManager is IOrderManager, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    constructor(IVivex _vivex, IWETH9 _nativeToken) {
        vivex = _vivex;
        positionNFT = _vivex.positionNFT();
        vault = _vivex.vault();
        positionFactory = _vivex.positionFactory();
        ...
        returns (PositionId positionId, Trade memory trade_, uint256 keeperReward)
    {
        ...
        Instrument memory instrument = vivex.instrument(symbol);
    ...
    {
        (positionId, trade_) = vivex.tradeOnBehalfOf(order.toTradeParams(), execParams, order.owner);
    }
        ...
        returns (PositionId positionId, Trade memory trade_)
    {
        ...
        (positionId, trade_) = vivex.trade(order.toTradeParams(), execParams);
